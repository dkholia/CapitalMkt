 ...
 * @see com.dcsoft.capmkt.orm.Person
...
public class PersonHome {
	...
	public Person findById(java.math.BigDecimal id) {
		log.debug("getting Person instance with id: " + id);
		try {
			Person instance = (Person) sessionFactory.getCurrentSession().get("com.dcsoft.capmkt.orm.Person", id);
			if (instance == null) {
	...
	public List<Person> findByExample(Person instance) {
		...
		try {
			List<Person> results = (List<Person>) sessionFactory.getCurrentSession()
					.createCriteria("com.dcsoft.capmkt.orm.Person").add(create(instance)).list();
